...
void Model::MakeCloth(std::vector<Particle*> particles, std::vector<PrimitiveTriangle*> triangles) {
	std::vector<ModelVertex> vtx;
	for (uint i = 0; i < particles.size(); i++) {
		vtx.push_back({ particles[i]->getPosition(), particles[i]->getNormal() });
	}

	std::vector<uint> idx;
	for (uint i = 0; i < triangles.size(); i++) {
		idx.push_back(triangles[i]->i1);
		idx.push_back(triangles[i]->i2);
		idx.push_back(triangles[i]->i3);
	}
	SetBuffers(vtx, idx);
}
